...
def load_tips():
    ...
    for row in rows[4:]:
        ...
        for p in participants:
            tip = row[p["col_index"]]
            if tip in ("1", "X", "2", 1, 2):
                tips_per_participant[p["name"]] = str(int(tip)) if isinstance(tip, (int, float)) else str(tip)
        match_tips.append({
...
def calculate_scores(results, match_tips):
    ...
    for result in results:
        for mt in match_tips:
            result_teams = set(result["match"].replace("–", " ").split())
            excel_teams = set(mt["match"].replace("–", " ").replace("-", " ").split())
            if result_teams & excel_teams:
...
def generate_blog_post(results, scores, persona):
    ...
    if scores:
        ...
        scores_text = "Tipsdata kunde inte matchas mot resultat."
    ...
    prompt = f"""Skriv ett blogginlägg om de senaste VM-resultaten ({today}).
...
- Kommentera tipsställningen med namn och poäng
- Lyfta fram Johan Nilssons rad specifikt
- Vara ca 300-400 ord
...
def main():
    ...
    print("Läser tipsrader...")
    participants, match_tips = load_tips()
    print(f"Hittade {len(participants)} deltagare och {len(match_tips)} matchrader.")
    print("Beräknar poäng...")
    scores = calculate_scores(results, match_tips)
    print(f"Poäng beräknade för {len(scores)} deltagare.")
